...
export default class ISPImporter {
  ...
  async import(): Promise<void> {
    ...
    const cables = this.convertISPFormatToCables(data.cables, boxes);
    const dropCables = this.convertISPFormatToDropCables(data.dropCables, boxes);
    const customers = this.convertISPFormatToCustomers(data.customers, dropCables);
  }

  private async fetchData(): Promise<ISPIntegrationData> {
    return Promise.all([
